...
from merger.classifier import Classifier
...
arg_parser.add_argument('-m', '--checkpoint-path', '--model-path', type=str, default='pointcnn.pth',
...
arg_parser.add_argument('-b', '--batch', type=int, default=8,
                        help='Batch size.')
arg_parser.add_argument('-p', '--prediction-output', type=str, default='guitar_pointcnn.npz',
...
net = Classifier(40).to(ns.device)
...
for i in tqdm.tqdm(range(0, len(kpn_ds), ns.batch), unit_scale=ns.batch):
    ...
    with torch.no_grad():
         input = torch.Tensor(np.array(Q)).to(ns.device)#.permute(0,2,1)
         key_points = net(input.to(ns.device)).permute(0,2,1)
